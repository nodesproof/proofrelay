    ...
    <div className="lower-grid">
      <div className="evidence-section">
        <section className="section-block" id="evidence">
          ...
          <p className="section-disclosure">
            ...
            {/*
              * The same name this page publishes as the ClaimReview author, read
              * from the host rather than written in, so a fork signs its own
              * ratings. Naming a publisher obliges one to be reachable when the
              * rating is wrong, so the route to say so is beside the name — not
              * in a footer under it.
              */}
            <span className="disclosure-by">
              Published by <strong>{typeof window === "undefined" ? "this deployment" : window.location.host}</strong>.{" "}
              <a href="https://github.com/nodesproof/proofrelay/blob/main/docs/CORRECTIONS.md" target="_blank" rel="noopener noreferrer">Report a verdict you think is wrong</a>
            </span>
          </p>
        ...
        <section className="section-block" id="reports">
          ...
          <div className="signal-table report-table"><div className="signal-head"><span>Verifier</span><span>Report hash</span><span>Model</span><span>Revealed</span><span /></div>{!task ? <SignalRowsSkeleton rows={2} /> : task.reports.length === 0 ? <EmptyState icon="inbox">No verifier has been assigned to this task yet.</EmptyState> : task.reports.map((report) => { const state = reportState(report); return <button className="signal-row" key={report.verifier} title={report.reportHash ? "Open the raw report artifact" : "No report has been revealed"} disabled={!report.reportHash} onClick={() => report.reportHash && setDrawer({ hash: report.reportHash, verifier: report.verifierLabel })}><div><span className={`signal-icon signal-${state.tone === "ink" ? "sky" : state.tone}`}><FileJson2 size={13} /></span><strong>{report.verifierLabel}</strong><Pill tone={state.tone}>{state.label}</Pill></div><span className="mono-text">{report.reportHash ? shortHash(report.reportHash) : report.commitment ? `commit ${shortHash(report.commitment)}` : "—"}</span><span className="model-cell" title={report.modelId ? `${report.modelId}${report.pipelineVersion ? ` · pipeline ${report.pipelineVersion}` : ""}` : undefined}><strong>{report.modelId ?? "—"}</strong>{/*
              * Where it ran, beside what ran it. The artifact has carried this
              * since the first mainnet task and nothing rendered it, so a page
              * that sells TEE-attested inference showed none of it — while a
              * "VERIFIED: TRUE" pill in the drawer, which is a content-hash
              * comparison and says nothing about a TEE, sat where a reader
              * would take it for one.
              *
              * "per router" is not hedging. The provider directory exposes no
              * quote and no measurement, so nothing here can be checked against
              * an attestation service; the schema calls it attribution rather
              * than verification, and a green tick would claim more than exists.
              */}
              {report.teeVerified === true && report.teeType ? <small className="tee-note" title="The router affirmed a TEE attestation for this completion. The provider directory publishes no quote or measurement, so this is the router's word, not an independently checkable proof.">{report.teeType} · per router</small>
                : report.teeVerified === false ? <small className="tee-note tee-none" title="No TEE attestation accompanied this completion — either the verifier did not ask for one, or the router did not affirm it.">no TEE attestation</small>
                : report.pipelineVersion ? <small>pipeline {report.pipelineVersion}</small> : null}</span><span className="muted-time"><Clock3 size={12} />{report.revealedAt ? relativeTime(report.revealedAt, new Date(now)) : report.committedAt ? `committed ${relativeTime(report.committedAt, new Date(now))}` : "—"}</span><ArrowUpRight size={14} /></button>; })}</div>
        </section>
      ...
      <div className="right-column">
        <div className="side-card">
          ...
          {wallet.isConnected && !wallet.isWrongNetwork && hasAllocation && <button className="primary-button full-button" onClick={runClaim} disabled={claimReward.isPending || windowOpen} title={windowOpen ? "Allocations unlock when the dispute window closes." : undefined}><WalletCards size={16} />{claimReward.isPending ? "Claiming…" : `Claim ${formatToken(allocationWei as bigint)}`}</button>}
            {wallet.isConnected && !wallet.isWrongNetwork && hasAllocation && windowOpen && task?.disputeDeadline && <div className="side-note">Your {formatToken(allocationWei as bigint)} is allocated but locked until the dispute window closes at {new Date(task.disputeDeadline).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}. Claiming before then reverts.</div>}
