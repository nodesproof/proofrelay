...
import type { Bytes32, ClaimView, ReportView, Tone, Verdict } from "@/lib/types";
...
function Row({ label, children }: { label: string; children: React.ReactNode }) { return <div><span>{label}</span><strong>{children}</strong></div>; }
...
export default function TaskDetail() {
  ...
  return <DashboardLayout eyebrow="Evidence and settlement record" title={heading}>
    <section className="section-block">
      <div className="section-header"><div><div className="eyebrow">TASK · {task ? task.ref : <Skeleton width={58} height={8} />}</div><h2>{task ? task.title : <Skeleton width={320} height={22} />}</h2></div><div className="section-tools">{task?.syncRequired && <button className="quiet-button" onClick={() => sync.mutate()} disabled={sync.isPending}>{sync.isPending ? "Re-indexing…" : "Index behind chain · re-index"}</button>}<button className="quiet-button" onClick={() => copy("Task ID", task?.taskId ?? handle)}><Copy size={13} /> Copy task ID</button><button className="quiet-button" onClick={() => open(task?.tx.explorerUrl)} disabled={!task?.tx.explorerUrl}>Creation tx <ArrowUpRight size={14} /></button></div></div>
      <div className="task-banner"><div className="task-banner-icon"><FileCheck2 size={20} /></div><div className="task-banner-copy"><div className="task-banner-top"><strong>{task ? task.title : <Skeleton width={260} height={13} />}</strong>{task ? <Pill tone={task.tone}>{task.status}</Pill> : <Skeleton className="sk-pill" width={78} />}{task && <span className="origin-chip origin-chain">{task.rawStatus} · {task.outcome}</span>}</div><span title={task?.question || undefined}>{task ? task.question || `${task.claimCount} claims · ${task.sourceCount} sources` : <Skeleton width={420} height={9} />}</span></div><div className="task-banner-hash"><span>Task ID</span><strong onClick={() => copy("Task ID", task?.taskId)}>{task ? shortHash(task.taskId) : <Skeleton width={72} height={9} />}</strong><span>Manifest hash</span><strong onClick={() => copy("Manifest hash", task?.manifestHash)}>{task ? shortHash(task.manifestHash) : <Skeleton width={72} height={9} />}</strong></div></div>
    </section>
    ...
    <div className="lower-grid">
      ...
      <div className="right-column">
        ...
        <div className="side-card">
          ...
          <div className="operator-metrics">{!task ? <><Row label="Status"><Skeleton width={78} height={9} /></Row><Row label="Creator"><Skeleton width={78} height={9} /></Row><Row label="Bounty"><Skeleton width={78} height={9} /></Row></> : <><Row label="Status">{task.rawStatus} · {task.outcome}</Row><Row label="Creator"><button className="quiet-button tiny-quiet" onClick={() => open(`${explorer.replace(/\/$/, "")}/address/${task.creator}`)}>{shortAddress(toChecksumAddress(task.creator) ?? task.creator)}</button></Row><Row label="Bounty">{task.bountyFormatted}</Row><Row label="Verifiers">{task.committedCount}/{task.revealedCount}/{task.verifierCount} committed/revealed/required</Row><Row label="Commit deadline">{task.commitDeadline ? untilLabel(task.commitDeadline, now) : "—"}</Row><Row label="Reveal deadline">{task.revealDeadline ? untilLabel(task.revealDeadline, now) : "—"}</Row><Row label="Dispute deadline">{task.disputeDeadline ? untilLabel(task.disputeDeadline, now) : "—"}</Row><Row label="Manifest pointer">{shortHash(task.manifestPointer, 8, 6)}</Row><Row label="Result hash">{task.resultHash ? shortHash(task.resultHash) : "not settled"}</Row><Row label="Created">{relativeTime(task.createdAt, new Date(now))}{task.tx.blockNumber === null ? "" : ` · block ${task.tx.blockNumber.toLocaleString("en-US")}`}</Row></>}</div>
        ...
        <div className="side-card">
          <div className="side-card-head"><div><span className="eyebrow">CONSENSUS</span><h3>{task?.consensus ? task.consensus.outcome : "Not evaluated"}</h3></div><Origin kind="storage" /></div>
