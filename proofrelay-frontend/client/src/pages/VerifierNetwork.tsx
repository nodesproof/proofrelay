...
export default function VerifierNetwork() {
  ...
  // No `?? items[0]` fallback: `current` IS the drawer's open state, so falling
  // back to the first verifier would leave the panel open over the directory on
  // every load. Nothing is selected until a row is clicked.
  const current = useMemo(() => items.find((verifier) => sameAddress(verifier.address, selectedAddress)), [items, selectedAddress]);
  ...
  const { writeContractAsync, feeOverrides, preflight, confirm: confirmTx } = useTxRunner();
  ...
  //
  // The metadata modal opens from inside the drawer, so Esc unwinds the top
  // layer first: closing the drawer under an open modal would leave the modal on
  // screen with the page it belongs to already gone.
  useEffect(() => {
  ...
  const submitRegistration = async () => {
    ...
    setBusy("register");
    try {
      const request = { address: PROOFRELAY_ADDRESS, abi: proofRelayAbi, functionName: "registerVerifier", args: [form.metadataHash.trim() as Bytes32, form.metadataPointer.trim()], value: stakeWei() } as const;
  ...
  const toggleActive = async () => {
    ...
    setBusy("active");
    try {
      const request = { address: PROOFRELAY_ADDRESS, abi: proofRelayAbi, functionName: "setVerifierActive", args: [next] } as const;
      const [fees, gas] = await Promise.all([feeOverrides(), preflight(request)]);
      const hash = await writeContractAsync({ ...request, chainId: ACTIVE_CHAIN_ID, ...fees, gas });
  ...
  const toggleApproval = async () => {
    ...
    setBusy("approval");
    try {
      const request = { address: PROOFRELAY_ADDRESS, abi: proofRelayAbi, functionName: "setVerifierApproval", args: [current.address, next] } as const;
      const [fees, gas] = await Promise.all([feeOverrides(), preflight(request)]);
      const hash = await writeContractAsync({ ...request, chainId: ACTIVE_CHAIN_ID, ...fees, gas });
  ...
  return <DashboardLayout eyebrow="Network operations" title="Verifier network">
    ...
    {registerOpen && <div className="modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setRegisterOpen(false); }}><div className="modal-card"><div className="modal-head"><div><span className="eyebrow">Verifier registration</span><h2>Register, then wait for the allow-list.</h2></div><button className="icon-button" onClick={() => setRegisterOpen(false)}>×</button></div><p className="modal-copy">This signs <strong>registerVerifier(bytes32,string)</strong> against {PROOFRELAY_ADDRESS} and records your address, the canonical hash of your metadata object and the pointer to it. Registration alone does not put you to work: ProofRelay's MVP sybil defence is an admin allow-list, so a DEFAULT_ADMIN holder must still call setVerifierApproval(you, true) before the dispatcher assigns you a task. Until they do, the directory lists you as PENDING.</p><div className="form-grid"><label className="field full"><span>Metadata hash (bytes32)</span><input value={form.metadataHash} onChange={(event) => setForm({ ...form, metadataHash: event.target.value })} placeholder="0x… 32-byte canonical hash of the metadata object" spellCheck={false} autoComplete="off" /></label><label className="field full"><span>Metadata pointer{pointerLimit === null ? "" : ` (max ${pointerLimit} bytes)`}</span><input value={form.metadataPointer} onChange={(event) => setForm({ ...form, metadataPointer: event.target.value })} placeholder="local://… under the local driver, or the 0G Storage root" spellCheck={false} autoComplete="off" /></label><label className="field"><span>Stake to lock</span><div className="input-with-suffix"><input value={form.stake} onChange={(event) => setForm({ ...form, stake: event.target.value })} placeholder="0" inputMode="decimal" autoComplete="off" /><span>0G</span></div></label><label className="field"><span>Minimum stake</span><div className="field-static"><span>{minVerifierStake === null ? "reading params()…" : formatToken(minVerifierStake)}</span><ShieldCheck size={15} /></div></label></div>{wallet.isConnected && registerBlocker && <p className="modal-error">{registerBlocker}</p>}<div className="modal-foot"><span className="modal-note"><CheckCircle2 size={15} />{slashBps === null ? "Approval by an admin is required after registration" : slashBps === 0 ? "Slashing disabled (slashBps 0) · approval by an admin still required" : `Slash rate ${slashBps / 100}% · approval by an admin still required`}</span><button className="primary-button" onClick={submitRegistration} disabled={busy === "register"}>{busy === "register" ? "Signing…" : !wallet.isConnected ? "Connect wallet" : wallet.isWrongNetwork ? `Switch to ${NETWORK_NAME}` : "Register verifier"} <ArrowUpRight size={16} /></button></div></div></div>}
        ...
        One surface now: opened by the row, closed by Esc, the backdrop or ×. */}
    {current && <div className="modal-backdrop drawer-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) setSelectedAddress(null); }}>
      <aside className="operator-drawer" role="dialog" aria-modal="true" aria-label={`Operator ${current.name}`}>
        <div className="side-card-head">
          <div><div className="eyebrow">SELECTED OPERATOR</div><h3>{current.name}</h3><div className={`directory-status ${statusTone(current.status)}`} title={`registered ${current.registered} · approved ${current.approved} · active ${current.active}${current.lastSeenAt ? ` · last reveal ${current.lastSeenAt}` : " · no reveal yet"}`}><span className="pill-dot" />{current.status}</div></div>
          <button className="icon-button" onClick={() => setSelectedAddress(null)} aria-label="Close operator panel">×</button>
        </div>
        <div className="operator-identity"><div className={`verifier-avatar large-avatar ${avatarTone(current.tone)}`}>{addressInitials(current.address)}</div><div><strong title={checksummedCurrent ?? current.address}>{current.shortAddress}</strong><span>{current.role}</span></div></div>
        <div className="operator-metrics">
          <div><span>Model version</span><strong title={current.modelId ? `${current.modelId}${current.pipelineVersion ? ` · pipeline ${current.pipelineVersion}` : ""} — from this verifier's latest revealed report` : "modelId only exists after revealReport"}>{modelLabel(current) ?? "No report revealed yet"}</strong></div>
          <div><span>Stake locked</span><strong className={chainRecord ? "chain-value" : undefined} title={chainRecord ? `getVerifier(${checksummedCurrent ?? current.address}).stake — read from chain ${ACTIVE_CHAIN_ID}${chainRecord.slashed > 0n ? ` · slashed ${formatToken(chainRecord.slashed)}` : ""}` : "from the read model; the onchain read has not landed yet"}>{chainRecord ? formatToken(chainRecord.stake) : onchain.isPending ? <Skeleton width={64} height={9} /> : current.stakeFormatted}</strong></div>
          <div><span>Median latency</span><strong title="Median of compute[].latencyMs across this verifier's revealed reports — self-reported">{latencyLabel(current.medianLatencyMs) ?? "Not reported"}</strong></div>
        </div>
        {/* What the "…" dropdown carried: read live from getVerifier(), so these
            three are the allow-list itself rather than the indexer's copy. */}
        <div className="operator-metrics">
          <div><span>Approved</span><strong className={chainRecord ? "chain-value" : undefined}>{chainRecord ? String(chainRecord.approved) : <Skeleton width={38} height={9} />}</strong></div>
          <div><span>Active</span><strong className={chainRecord ? "chain-value" : undefined}>{chainRecord ? String(chainRecord.active) : <Skeleton width={38} height={9} />}</strong></div>
          <div><span>Slashed</span><strong className={chainRecord ? "chain-value" : undefined}>{chainRecord ? formatToken(chainRecord.slashed) : <Skeleton width={58} height={9} />}</strong></div>
        </div>
        <div className="uptime-panel"><div><span>30-day uptime</span><strong>{percentLabel(current.uptimePct, 1) ?? "—"}</strong></div><div className="uptime-bars">{buckets.map((value, index) => <i key={index} style={{ height: value === null ? "8%" : `${Math.max(3, Math.min(100, value))}%` }} className={value === null ? "bar-empty" : value < UPTIME_WARNING_PCT ? "bar-warning" : ""} title={value === null ? "No commits in this bucket" : `${value.toFixed(1)}% revealed`} />)}</div></div>
